...
```{r, eval = FALSE}
papers <- read("papers")
search_text(papers, "Canadian")
```
::: {.module}

```{r, echo = FALSE}
papers <- read("papers")
search_text(papers, "Canadian") |> knitr::kable()
```
...
```{r, eval = FALSE}
search_text(papers, "\\b\\S+\\s*=\\s*\\S+\\b", return = "match")
```

::: {.module}

```{r, echo = FALSE}
search_text(papers, "\\b\\S+\\s*=\\s*\\S+\\b", return = "match") |> head(10) |> knitr::kable()
```
...
```{r}
#| eval: false
papercheck::llm_model_list()
```
::: {.module}

```{r}
#| echo: false
papercheck::llm_model_list() |> knitr::kable()
```
